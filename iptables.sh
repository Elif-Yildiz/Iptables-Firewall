...
sudo ip netns exec firewall iptables -Fc # flush the prev rulezzzz dummy
...
sudo ip netns exec firewall iptables -A INPUT -p icmp -s ${c1_ip} -i veth-fw-c1 -d ${c1_fw_ip} -j DROP #REJECT #default policye değilde bu kurala takıldığının anlamak için reject yaptım, normalde drop olarak kullanacağım
